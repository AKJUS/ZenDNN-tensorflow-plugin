...
__The latest ZenDNN Plugin for TensorFlow* (zentf) 5.2.2 is here!__

The ZenDNN plugin for TensorFlow is called zentf.

The zentf 5.2.2 plugin works seamlessly with TensorFlow version 2.21.0, offering a high-performance experience for deep learning on AMD EPYC™ platforms.
...
* Create conda environment and activate it.
  ```
  conda create -n tf-v2.21.0-zendnn-v5.2.2-rel-env python=3.10 -y
  conda activate tf-v2.21.0-zendnn-v5.2.2-rel-env
  ```
...
* Run the following commands to unzip the package and install the binary.
  ...
  ```
  unzip ZENTF_v5.2.2_Python_v3.10.zip
  cd ZENTF_v5.2.2_Python_v3.10/
  pip install zentf-5.2.2-cp310-cp310-manylinux_2_28_x86_64.whl
  ```
...
```
Note: Repository defaults to main branch, to build the version 5.2.2 checkout the r5.2.2 branch.
```
git checkout r5.2.2
```
...
```
ZenDNN-tensorflow-plugin$ pip install zentf-5.2.2-cp310-cp310-linux_x86_64.whl
```
...
* zentf v5.2.2 is supported with ZenDNN v5.2.2. For detailed performance tuning guidelines, refer to the [Performance Tuning](https://docs.amd.com/r/en-US/57300-ZenDNN-user-guide/Performance-Tuning) section of the ZenDNN user guide.
