...
**Note:** zentf 5.2.2 supports TensorFlow-Java v1.2.0-SNAPSHOT (TensorFlow v2.20.0), which must be built from source.
...
### Set-up zentf plugin
Download zentf C++ plugin package build with TensorFlow-v2.20 for 5.2.2 release for Java from [AMD Developer Forum](https://www.amd.com/en/developer/zendnn.html).

Set the environment variable 'LD_LIBRARY_PATH' with the path to zentf plugin libraries.
```bash
unzip ZENTF_v5.2.2_C++_API.zip
export LD_LIBRARY_PATH=<Path to zentf C++ parent folder>/ZENTF_v5.2.2_C++_API/lib-tensorflow-plugins
```

Set ZenDNN specific environment variables as shown below,
```bash
cd <Path to zentf C++ parent folder>/ZENTF_v5.2.2_C++_API
