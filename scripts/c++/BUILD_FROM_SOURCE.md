...
This guide walks through building the TensorFlow-ZenDNN plug-in C++ package (`ZENTF_v2.21.0.0_C++_SOURCE_BUILD`) from source. The package enables ZenDNN-accelerated inference via the TensorFlow C++ API on AMD CPUs.
...
```bash
cd <workspace>
mkdir -p ZENTF_v2.21.0.0_C++_SOURCE_BUILD/lib-tensorflow-plugins

# Plug-in library
cp ZenDNN_TensorFlow_Plugin/bazel-bin/tensorflow_plugin/libamdcpu_plugin_cc.so \
   ZENTF_v2.21.0.0_C++_SOURCE_BUILD/lib-tensorflow-plugins/
...
cp ZenDNN_TensorFlow_Plugin/scripts/c++/zentf_cc_api_setup.sh  ZENTF_v2.21.0.0_C++_SOURCE_BUILD/
cp ZenDNN_TensorFlow_Plugin/scripts/zentf_env_setup.sh         ZENTF_v2.21.0.0_C++_SOURCE_BUILD/
cp ZenDNN_TensorFlow_Plugin/examples/c++/sample_inference.cpp   ZENTF_v2.21.0.0_C++_SOURCE_BUILD/
```
...
unzip tf_wheel_tmp/tensorflow*.whl -d "ZENTF_v2.21.0.0_C++_SOURCE_BUILD/tensorflow_${TF_VERSION}"
rm -rf tf_wheel_tmp
```
...
```bash
cd "ZENTF_v2.21.0.0_C++_SOURCE_BUILD/tensorflow_${TF_VERSION}/tensorflow"
...
```bash
cd <workspace>/ZENTF_v2.21.0.0_C++_SOURCE_BUILD
